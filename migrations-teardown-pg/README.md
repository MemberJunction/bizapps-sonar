# Teardown scripts (PostgreSQL)

The PostgreSQL twin of `migrations-teardown/`. MemberJunction's Open App engine
prefers `<teardownDirectory>-pg/` when the target is PostgreSQL and falls back to
the declared directory if this one is missing, so removing it would send SQL
Server `[bracket]` quoting to a PostgreSQL server and abort the uninstall on its
first statement.

Generated by the same `ci/generate_teardown.mjs` run that writes the SQL Server
