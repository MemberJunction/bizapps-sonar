...
- `FactorCompiler` + `CompiledFactorEvaluator` — translate a declarative factor's config into one set-based SQL query and run it (v1: Count/Sum/Avg/Min/Max/DistinctCount aggregations with config-field validation, single-hop foreign-key join, rolling time window).
