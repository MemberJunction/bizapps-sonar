...
- Rolling windows now accept a length in **months** (`LengthMonths` → `DATEADD(month, -N, @asOf)`), not just days — so month-based windows like "Trailing 12 Months" compile. (Where a Rolling window's *date column* comes from is the separate `Factor.DateField` decision, marked in the roadmap for the next major migration; the `AnchorDateField` bridge remains the interim.)
