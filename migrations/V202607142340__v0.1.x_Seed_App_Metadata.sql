...
-- Save MJ: AI Agent Actions (agent tool surface)
-- Bind the Sonar Authoring Agent to its authoring actions. WITHOUT these links the
-- Loop-agent runtime builds an empty toolbox and every action call reports
-- "unavailable" — the NL authoring agent is dead on a fresh install. Idempotent:
-- guarded on (AgentID, ActionID) so a re-run never duplicates a link.
-- PLACEMENT: must run AFTER all Sonar Actions are seeded (ActionID FK) — keep at end of file.
