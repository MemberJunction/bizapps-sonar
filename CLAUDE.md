...
| Package | Tier | Holds |
|---|---|---|
| `sonar-engine` | server | ScoringEngine, FactorCompiler, NormalizationEngine, RecomputeOrchestrator, SegmentEvaluator |
...
- **Never widen a server package's audience to share domain logic with the client.** Add or extend a read-only Action (`Sonar: Explain Scores`, `Sonar: Preview Segment` both exist for exactly this). If the browser genuinely must compute something, it goes in a package with no dependencies and no MJ peers, declared `shared` in `mj-app.json`.
- **Why this is a critical rule**: "which signal is dragging this member down" once existed three times — the Triage Why column, the Movers Why column, and a private copy inside the Draft Outreach play. They silently diverged when the calculation was corrected to rank on the rubric weight, and the play hands its label to an LLM as fact, so the stale copy would have asserted something false in a message to a real person.
...
- **`PercentOfTotal` is a trap.** The scorer writes it as 0 for a factor the member had no data on — exactly when that factor is hurting them most. Anything reasoning about *why* a score is low must rank on the configured `ModelFactor.Weight` instead. See `packages/Engine/src/scoring/factorDrag.ts`.
