...
full population that's N calls per recompute — 10k members ⇒ 10k calls. The parallelism bound
(`MaxConcurrency`) plus a loud `LogStatus` above a soft cap (`ACTION_FACTOR_POPULATION_SOFT_CAP`, ~1000)
is a stopgap, **not** a guard. Deferred (designed in `plan.md` §5 schema + §6.1): result caching
(`CacheTTLSeconds`), `IsExpensive` budgeting + graceful degradation (skip + mark stale),
`RateLimitPerMinute`, and Batch mode. (Action-backed factors are out of v1's *built* scope — see §5.2,
`⬜ Only Declarative implemented` — so this is a Phase-1 decision recorded ahead of the build.)
