...
- **`src/lib/generated/`** — CodeGen-owned form components and the tree-shaking-prevention manifest. Never edit by hand; regenerated by `pnpm run mj:codegen` / `mj codegen manifest`.
...
```bash
cd packages/Angular && pnpm run build  # ngc
```
