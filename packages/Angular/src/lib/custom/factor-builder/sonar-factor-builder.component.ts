...
import { FactorService, Aggregation, WeightMode } from "../services/factor.service";
...
export class SonarFactorBuilderComponent {
    ...
    /** Serialize the filter only when it actually has conditions (else "include everything"). */
    private filterExpression(): string | undefined {
        const f = this.filter();
        return f.filters && f.filters.length > 0 ? JSON.stringify(f) : undefined;
    }
            ...
            const ok = await this.factorService.createAndBind(
                {
                    ...
                    normalizationMethod: this.normalization() === "None" ? "None" : "MinMax",
